...
Do not answer the generated nodes' domain tasks while authoring or fabricate worker results. Simulation fixtures are examples, never completed execution evidence. Prompts and human answers do not grant execution permissions. New runs require an operator-supplied repository working directory (`--working-directory` in the start CLI, `workingDirectory` in the start API). Real workers can modify that workspace under the recorded workspace-write sandbox. All nodes share the selected directory; supply a common parent for multi-repository workflows. Do not invent YAML permission or cwd fields. Existing histories without the binding remain read-only. Builds may still require unavailable caches, dependencies, network access, or devices; report actual blockers. File edits and shell commands may repeat after interruptions; committed outputs are not exactly-once side effects.

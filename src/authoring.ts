...
- needs (or depends_on, never both) references sibling nodes and each scope graph must be acyclic. A fan-in node lists every dependency.
- Executable groups use kind: scope, inputs, a non-empty nested nodes mapping, and outputs mapping export names to bindings such as $nodes.review.output. They omit agent prompts. A scope without loop executes once; scopes may contain agents, humans, and other scopes. groups remains named concurrency limits, not executable grouping.
- Scope loop requires explicit max_iterations 1 through 20, on_exhaustion (fail or accept_last), initial state mapping, next state mapping, and until. initial resolves from the scope inputs; next resolves $output (that iteration's exports), $state (old state), and $input. next must supply exactly the initial state's keys. Scope children see local $input, $state from the nearest enclosing loop, and sibling $nodes outputs. External consumers see only scope exports. Do not use carry_as on a scope.
- Scope loop optionally accepts agent_sessions: fresh or resume (default fresh). Resume reuses a separate recorded provider conversation per descendant leaf and per mapped source position across that scope's iterations. First encounters start fresh; missing recorded session IDs explicitly start fresh. A loop-free scope inherits the enclosing policy; any nested repeated scope owns its own policy (default fresh) and its session map resets for each activation. Do not add agent_sessions to agent loops or non-loop scopes.
- Session conversation is separate from workflow data: always declare loop.next and input bindings for data handoffs. Resume allows changed iteration inputs but requires the same actual provider and canonical local workspace. It is not cross-workspace portability. Resume failure uses operator recovery; an explicit successful fresh-session retry replaces the session used by later iterations. Session reuse grants no additional execution permissions.
- Each scope iteration waits for every child commit before exporting and evaluating until. False repeats the entire scope. Both parallel agents must succeed in the same iteration for an all condition. Failure/recovery differs from a valid false result. Scopes nest to at most eight levels; runtime expansion is bounded to 1000 instances.
- Example graph: scope resolve contains review then scope parallel_work whose implementation and validation agents both depend on the review guidance through parallel_work inputs. Export implementation and validation, use until.all with paths implementation.success and validation.success equals true, carry both results to review through initial/next, then release next_agent depending on resolve.
- Input bindings use $input.field or $nodes.node_id.output.field. Prefer listing every $nodes source directly in needs; references to transitive ancestors are also supported through declared dependency chains.
- Agent loop supports max_iterations 1 through 20, carry_as, on_exhaustion (fail or accept_last), and until with path/operator/value. Agent and scope predicates also compose with non-empty all/any arrays or not. Operators: equals, not_equals, greater_than, greater_than_or_equal, less_than, less_than_or_equal, contains, truthy.

...
3. Each node runs as an Activity. In `simulated` mode it emits deterministic demo output. In `codex` mode new runs launch `codex exec --sandbox workspace-write` in their mandatory, canonical repository working directory with a generated JSON Schema and resolved inputs. The start client validates the directory before connecting to Temporal; the binding is retained in Workflow input, `execution.json`, projected state, and hashed agent receipts. Fresh and resumed provider subprocesses use the same directory. Historical runs without the binding retain the previous read-only behavior. The Activity parses `thread.started`, immediately heartbeats the provider session, and uses `codex exec resume` on a matching automatic retry.
4. Agent output validation and receipt commit happen before the result becomes available to downstream nodes. Human nodes wait for a validated `submitHumanInput` Update. Human gates inside executable scopes commit immutable `human-output.json` evidence for the `{answer}` result before releasing dependents. Top-level human gates retain their existing event-only commit path to preserve supported histories. The nested human artifact is separate from a provider completion receipt.
...
## Executable scopes and repeat

V1 `kind: scope` groups a nested graph behind explicit inputs and exported outputs (ADR-019). A scope without `loop` executes once. `depends_on` is an alias for `needs` (never both); references to committed transitive ancestors remain supported through declared dependency chains. A scope loop runs review, parallel work, human gates, or nested scopes to completion before committing its exported output and evaluating a deterministic `until`. `all`/`any`/`not` compose existing predicates. Explicit `initial` and `next` state bindings carry results between iterations. A false result repeats the complete scope; only a terminal accepted result releases outside dependents.

The interpreter registers dynamic child instances with full enclosing iteration paths, for example `resolve~2.parallel_work~1.validation`. Their receipts, transitions, human requests, and recovery controls retain those identities. Scope exports are committed by an administrative Activity under `nodes/<scope-instance>/scope-iterations/<iteration>/output.json`, with run identity, child evidence references and output hashes, and an export schema hash before condition evaluation. The commit Activity validates child evidence and recomputes the exported bindings before immutable publication. This is the current file-based evidence path, not the target transactional store. Scope nesting is limited to eight levels and runtime expansion to 1000 instances. Scopes consume no provider capacity themselves.

Console keeps scopes as atomic top-level graph nodes. The inspector lists child instances across iterations, with navigation into nested scopes and the existing human/recovery forms addressed by qualified IDs. Build previews expose nested authored children through the scope detail drawer. Loop labels format the interpreter's outcome; historical records without an outcome display unknown. The browser never evaluates the predicate against output.

The portable [parallel review example](../skills/steward-workflow/assets/scope-loop.yaml) repeats review and two parallel agents until both succeed in the same iteration, then runs `next_agent`. Simulation fixtures exercise iteration selection; they do not establish real provider quality.

## Scope session policy

Repeated scopes optionally declare `loop.agent_sessions: resume` (ADR-020); omitted or explicit `fresh` preserves fresh conversations across scope iterations. Resume retains one provider session per descendant leaf and mapped source position within that repeated-scope activation. Non-loop scopes propagate the policy; a nested repeat owns an independent policy and affinity map, reset for every outer activation. A leaf's own loop/retry behavior remains unchanged.

Affinity is accepted-result state in the interpreter, with provider, canonical workspace, and session ID bound into receipt/result and heartbeat/recovery metadata. The Activity canonicalizes and checks workspace/provider identity before resuming. The first encounter and missing session ID explicitly start fresh via `node.session`. Resume failures use the existing recovery choices; successful fresh-session recovery replaces the affinity for subsequent iterations. Session continuity does not replace `loop.next` or input bindings and does not provide workspace or machine portability.

The [opt-in example](../skills/steward-workflow/assets/scope-loop-resume.yaml) preserves the default example separately. Both examples select simulation; no live provider behavior is claimed from their offline validation.
...
### Prepared dashboard runs (ADR-023)

The Console loads `/api/workflows`, keeps its editable selection separate from run history, and calls `/api/runs/prepare` before `Start new run`. The preparation use case snapshots the compiled definition, input, selected repository and execution mode under the runtime directory. Starting with a prepared ID submits that snapshot with a stable Temporal identity; repeated starts cannot create another execution while that history is retained. Catalog-enabled Consoles hide the old configured-workflow shortcut. Legacy clients may still use the existing configured `/api/runs` request.

`workflow` mode respects each declared agent provider; `simulated` overrides all agent nodes and never launches Codex. Previews validate root input references; dependent outputs and scoped runtime values are checked during execution.

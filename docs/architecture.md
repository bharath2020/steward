...
Build mode places an agent chat rail beside the existing SVG graph on desktop and stacks them on narrow screens. `src/authoring.ts` supplies the V1 contract to either the installed Codex or Claude Code CLI, requires structured reply/YAML output, and invokes the same pure `parseWorkflow` function used by authored files. Invalid YAML receives one parser-diagnostic repair attempt and is otherwise shown as an error without replacing the last valid graph. The endpoint is bounded and same-loopback-origin, and providers run without write permission. Drafts are not stored and chatting cannot start Temporal; see ADR-013.
